...
static void
fillJsonRtpEncodingParameters(json &jsonEncoding,
...
Handler::Handler(PrivateListener *privateListener, const json &iceParameters,
  ...
  if (dtlsParameters.find("role") != dtlsParameters.end() &&
      dtlsParameters["role"].get<std::string>() != "auto") {
    this->forcedLocalDtlsRole =
        dtlsParameters["role"].get<std::string>() == "server" ? "client"
                                                              : "server";
  }

  this->pc.reset(new PeerConnection(this, peerConnectionOptions));

  this->remoteSdp.reset(new Sdp::RemoteSdp(iceParameters, iceCandidates,
...
void Handler::OnIceConnectionChange(
...
SendHandler::SendHandler(Handler::PrivateListener *privateListener,
                         const json &iceParameters, const json &iceCandidates,
                         const json &dtlsParameters, const json &sctpParameters,
                         const PeerConnection::Options *peerConnectionOptions,
                         const json &sendingRtpParametersByKind,
                         const json &sendingRemoteRtpParametersByKind)
    : Handler(privateListener, iceParameters, iceCandidates, dtlsParameters,
              sctpParameters, peerConnectionOptions) {
  MSC_TRACE();
...
SendHandler::SendResult
SendHandler::Send(webrtc::MediaStreamTrackInterface *track,
                  std::vector<webrtc::RtpEncodingParameters> *encodings,
                  const json *codecOptions, const json *codec) {
  MSC_TRACE();
  ...
  MSC_DEBUG("[kind:%s, track->id():%s]", track->kind().c_str(),
            track->id().c_str());
  ...
  webrtc::RtpTransceiverInterface *transceiver =
      this->pc->AddTransceiver(track, transceiverInit);
  ...
  try {
    ...
    if (!this->transportReady)
      this->SetupTransport(!this->forcedLocalDtlsRole.empty()
                               ? this->forcedLocalDtlsRole
                               : "server",
                           localSdpObject);
    ...
    std::string scalability_mode =
        encodings && encodings->size()
            ? ((*encodings)[0].scalability_mode.has_value()
                   ? (*encodings)[0].scalability_mode.value()
                   : "")
            : "";
    ...
    auto mimeType =
        sendingRtpParameters["codecs"][0]["mimeType"].get<std::string>();

    std::transform(mimeType.begin(), mimeType.end(), mimeType.begin(),
                   ::tolower);

    if (encodings && encodings->size() == 1 && spatialLayers > 1 &&
        mimeType == "video/vp9") {
  ...
  sendingRtpParameters["rtcp"]["cname"] =
      Sdp::Utils::getCname(offerMediaObject);

  // Set RTP encodings by parsing the SDP offer if no encodings are given.
  if (encodings == nullptr || encodings->empty()) {
    sendingRtpParameters["encodings"] =
        Sdp::Utils::getRtpEncodings(offerMediaObject);
  }
  ...
  this->remoteSdp->Send(offerMediaObject, mediaSectionIdx.reuseMid,
...
Handler::DataChannel
SendHandler::SendDataChannel(const std::string &label,
  ...
  if (dataChannelInit.maxRetransmitTime.has_value()) {
    sctpStreamParameters["maxPacketLifeTime"] =
        dataChannelInit.maxRetransmitTime.value();
  }

  if (dataChannelInit.maxRetransmits.has_value()) {
    sctpStreamParameters["maxRetransmits"] =
        dataChannelInit.maxRetransmits.value();
  }
  ...
  this->nextSendSctpStreamId =
      (this->nextSendSctpStreamId + 1) % SctpNumStreamsMis;
  ...
  if (!this->hasDataChannelMediaSection) {
    ...
    auto offerMediaObject =
        find_if(localSdpObject["media"].begin(), localSdpObject["media"].end(),
                [](const json &m) {
    ...
    MSC_DEBUG("calling pc.setRemoteDescription() [answer:%s]",
              sdpAnswer.c_str());
...
void SendHandler::ReplaceTrack(const std::string &localId,
  ...
  MSC_DEBUG("[localId:%s, track->id():%s]", localId.c_str(),
...
void SendHandler::SetMaxSpatialLayer(const std::string &localId,
                                     uint8_t spatialLayer) {
  MSC_TRACE();

  MSC_DEBUG("[localId:%s, spatialLayer:%" PRIu8 "]", localId.c_str(),
            spatialLayer);
...
RecvHandler::RecvHandler(Handler::PrivateListener *privateListener,
                         const json &iceParameters, const json &iceCandidates,
                         const json &dtlsParameters, const json &sctpParameters,
                         const PeerConnection::Options *peerConnectionOptions)
    : Handler(privateListener, iceParameters, iceCandidates, dtlsParameters,
...
RecvHandler::RecvResult RecvHandler::Receive(const std::string &id,
  ...
  if (midIt != rtpParameters->end() &&
      (midIt->is_string() && !midIt->get<std::string>().empty()))
  ...
  auto transceiverIt =
      std::find_if(transceivers.begin(), transceivers.end(),
                   [&localId](webrtc::RtpTransceiverInterface *t) {
...
Handler::DataChannel
RecvHandler::ReceiveDataChannel(const std::string &label,
                                webrtc::DataChannelInit dataChannelInit) {
  MSC_TRACE();
  ...
  if (!this->hasDataChannelMediaSection) {
    ...
    MSC_DEBUG("calling pc->setRemoteDescription() [offer:%s]",
              sdpOffer.c_str());
    ...
    MSC_DEBUG("calling pc->setLocalDescription() [answer: %s]",
              sdpAnswer.c_str());
