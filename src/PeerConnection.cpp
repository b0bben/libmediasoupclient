...
PeerConnection::PeerConnection(PeerConnection::PrivateListener *privateListener,
  ...
  if ((options != nullptr) && (options->factory != nullptr)) {
    ...
    if (!this->networkThread->Start() || !this->signalingThread->Start() ||
        !this->workerThread->Start()) {
      MSC_THROW_INVALID_STATE_ERROR("thread start errored");
    }

    this->peerConnectionFactory = webrtc::CreatePeerConnectionFactory(
        this->networkThread.get(), this->workerThread.get(),
        this->signalingThread.get(), nullptr /*default_adm*/,
        webrtc::CreateBuiltinAudioEncoderFactory(),
        webrtc::CreateBuiltinAudioDecoderFactory(),
        std::make_unique<webrtc::H264MFEncoderFactory>(),
        std::make_unique<webrtc::H264MFDecoderFactory>(),
  ...
  this->pc = this->peerConnectionFactory->CreatePeerConnection(
      config, nullptr, nullptr, privateListener);
}
...
bool PeerConnection::SetConfiguration(
  ...
  MSC_WARN("webrtc::PeerConnection::SetConfiguration failed [%s:%s]",
...
void PeerConnection::SetLocalDescription(PeerConnection::SdpType type,
  ...
  if (sessionDescription == nullptr) {
    MSC_WARN("webrtc::CreateSessionDescription failed [%s]: %s",
...
void PeerConnection::SetRemoteDescription(PeerConnection::SdpType type,
                                          const std::string &sdp) {
  MSC_TRACE();
...
void PeerConnection::SetSessionDescriptionObserver::Reject(
  ...
  this->promise.set_exception(
      std::make_exception_ptr(MediaSoupClientError(error.c_str())));
}
...
void PeerConnection::SetSessionDescriptionObserver::OnFailure(
  ...
  MSC_WARN("webtc::SetSessionDescriptionObserver failure [%s:%s]",
           webrtc::ToString(error.type()), error.message());
...
void PeerConnection::CreateSessionDescriptionObserver::OnFailure(
  ...
  MSC_WARN("webtc::CreateSessionDescriptionObserver failure [%s:%s]",
           webrtc::ToString(error.type()), error.message());
...
void PeerConnection::PrivateListener::OnSignalingChange(
  ...
  MSC_DEBUG("[newState:%s]",
            PeerConnection::signalingState2String[newState].c_str());
}
 ...
 * begun.
 */
void PeerConnection::PrivateListener::OnRenegotiationNeeded() { MSC_TRACE(); }
...
void PeerConnection::PrivateListener::OnIceConnectionChange(
  ...
  MSC_DEBUG("[newState:%s]",
            PeerConnection::iceConnectionState2String[newState].c_str());
}
...
void PeerConnection::PrivateListener::OnIceGatheringChange(
  ...
  MSC_DEBUG("[newState:%s]",
            PeerConnection::iceGatheringState2String[newState].c_str());
}
